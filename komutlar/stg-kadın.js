...
.setColor('#cc99cc')).then(x => x.delete({timeout: 5000}));
  
const tag = ''   
const kadınrol = message.guild.roles.cache.find(r => r.id === '709421783500980305') 
const kadınrol2 = message.guild.roles.cache.find(r => r.id === '800209722044317746')
...
.setAuthor(message.author.tag, message.author.avatarURL({dynamic:true}))
.setDescription(`1. Kadın rolü ayarlanmamış / Yanlış id girilmiş kayıt işlemine devam edilemiyor. <a:cute13:796060385198866472>`)
.setColor('#cc99cc')).then(x => x.delete({timeout: 5000}));
...
genelchat.send(`${member} Aramıza katıldı, hoşgeldin umarım keyifli vakit geçirirsin. <a:kalpem:800219823735832586>`)

...
if(!["709069702659309568"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("ADMINISTRATOR"))) 
...
.setAuthor(message.author.tag, message.author.avatarURL({dynamic:true}))
.setColor("0x2f3136")
.setDescription(`${member} Adlı kullanıcının kayıtları <@${message.author.id}> tarafından sıfırlandı.`))
